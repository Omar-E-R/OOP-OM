...
public class TestGestionProtagonistes {
	...
	public static void main(String[] args) {
		...
		Monstre<Tranchant> vampirien = new Monstre<>("vampirien", 10, ZoneDeCombat.AERIAN, Domaine.TRANCHANT,
				new Morsure(10));
...
//		[Heros [nom=Arthur, forceDeVie=100], Heros [nom=Archibald, forceDeVie=100], Homme [nom=Thomas, forceDeVie=70], Homme [nom=Louis, forceDeVie=70], Homme [nom=Alain, forceDeVie=70]]
		...
		NavigableSet<Monstre<? extends Pouvoir>> monstres = aideEcrivain.getMonstreDeFeu();
		String affichage = affichageMonstres(monstres);
		System.out.println("\nmonstres de feu :\n" + affichage); 
		
		
		Monstre<Glace> soufflemort = new Monstre<>("soufflemort", 120,ZoneDeCombat.AERIAN, Domaine.GLACE, new Tornade(8));
	...
	public static String affichageMonstres(NavigableSet<Monstre<? extends Pouvoir>> monstres) {
		...
		for(Monstre<? extends Pouvoir> monstre : monstres)
			strBld.append(monstre.getNom()+" monstre de "+ monstre.getDomaine()+", ");
