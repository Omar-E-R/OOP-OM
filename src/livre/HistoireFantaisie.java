...
public class HistoireFantaisie {
	...
	public void creationMonstresDefaut() {
		...
		listeDragotenebre.add(new Monstre<Feu>("dragotenebre", 200, ZoneDeCombat.AERIAN, Domaine.FEU, new BouleDeFeu(4),
				new Lave(1), new Eclair(3)));
		monstres.put("dragotenebre", listeDragotenebre);

		List<Monstre<?>> listeVampirien = new ArrayList<>();
		for (int i = 0; i < 10; i++) {
			listeVampirien.add(new Monstre<Tranchant>("vampirien" + i, 10, ZoneDeCombat.AERIAN, Domaine.TRANCHANT,
					new Morsure(10)));
		}
		...
		listeSoufflemort
				.add(new Monstre<Glace>("soufflemort", 120, ZoneDeCombat.AERIAN, Domaine.GLACE, new Tornade(8)));
	...
	public void creationGrotteDefaut() {
		...
		ajouterSalle(ZoneDeCombat.AERIAN, "soufflemort");
	}
	...
	public void afficherLesMonstres() {
		...
		for(Map.Entry<String, List<Monstre<? extends Pouvoir>>> entry : monstres.entrySet()) {
			strBld.append(entry.getKey()+", ");
		}
		int taille=strBld.length();
		strBld.delete(taille - 2, taille-1);
		strBld.insert(taille-2, ".");
		livre.ecrire(strBld.toString());
	}
